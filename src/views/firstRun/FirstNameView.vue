<template>
  <first-config-layout
    :stepNumber="5"
    header-title="Imię"
    header-text="Podaj nam swoje imię w celu uzupełnienia twojego profilu"
  >
    ...
    <template v-slot:form>
      ...
      <form class="flex flex-col gap-8" @submit.prevent="pushDataToStorage">
        <div class="relative flex gap-4 items-center justify-center max-w-xs">
          <label for="weight" class="sr-only">Wprowadź swoje imie</label>
          <div class="relative">
            <input
              type="text"
              name="weight"
...
import NameIllustration from "../../components/illustrations/NameIllustration.vue";
